...
st.set_page_config(page_title="Kredi Uygunluğu",
                   page_icon=":bank:", layout="wide")

st.markdown("<h1 style='text-align: center; font-size: 40px;'>Tahmin Bank'a Hoşgeldiniz!</h1>",
...
Married_input = st.selectbox(
    label='Başvuru sahibi medeni hali', options=("Evli", "Bekar"))
Dependents_input = st.selectbox(
    label='Başvuru sahibinin bakmakla yükümlü olduğu kişi sayısı kaçtır?', options=("0", " 1", "2", "3+"))
Education_input = st.selectbox(
    label='Başvuru sahibinin eğitim seviyesi nedir?', options=("Lisansüstü", "Lisans"))
Self_Employed_input = st.selectbox(
    label='Başvuru sahibi kendi işinin sahibi midir?', options=("Evet", "Hayır"))
ApplicantIncome_input = st.slider(
    label='Başvuru sahibinin geliri ne kadardır?', min_value=0, max_value=100000)
CoapplicantIncome_input = st.slider(
    label='Başvuru sahibiyle birlikte başvuran kişinin geliri nedir?', min_value=0.0, max_value=100000.0)
LoanAmount_input = st.slider(
    label='İstenilen kredinin tutarı ne kadardır?', min_value=0.0, max_value=100000.0)
Loan_Amount_Term_input = st.slider(
    label='İstenilen kredinin vadesi ay cinsinden ne kadardır?', min_value=0.0, max_value=480.0, step=1.0)
Credit_History_input = st.selectbox(
    label='Başvuru sahibinin kredi geçmişi var mı?', options=(1.0, 0.0))
Property_Area_input = st.selectbox(
    label='Kredi istenilen mülk alanı nerededir?', options=("Yarı Kentsel", "Kentsel", "Kırsal"))
